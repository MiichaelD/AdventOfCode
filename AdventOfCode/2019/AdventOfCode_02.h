...
#ifndef _2019_ADVENTOFCODE_02_H_
#define _2019_ADVENTOFCODE_02_H_
...
namespace aoc2019_02 {  
  ...
  void processIntCodes(vector<int> &intCodes) {
    ...
    for (size_t i = 0; i < intCodes.size();) {
      intCode = intCodes[i];
      if (intCode > 0 && intCode < 3) {
        aux1 = intCodes[intCodes[i+1]];
        aux2 = intCodes[intCodes[i+2]];
        intCodes[intCodes[i+3]] = intCode == 1 ? (aux1 + aux2) : (aux1 * aux2);
        i += 4;
      } else {
        return;
      }
  ...
  void solve1() {   
    ...
    printIntCodes(intCodes);
    cout << intCodes[0] << endl;
  }
  ...
  void solve2() {    
    ...
    for (int i = 0; i < 100; ++i) {
      for (int j = 0; j < 100; ++j) {
        vector<int> intCodesCopy = intCodes;
        intCodesCopy[1] = i;
        intCodesCopy[2] = j;
        processIntCodes(intCodesCopy);
        int output = intCodesCopy[0];
        // cout << output << endl;
        if (output == 19690720) {
          cout << (100 * i + j) << endl;
          return;
...
#endif /* _2019_ADVENTOFCODE_02_H_ */
