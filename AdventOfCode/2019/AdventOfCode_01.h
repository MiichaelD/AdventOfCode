...
#ifndef _2019_ADVENTOFCODE_01_H_
#define _2019_ADVENTOFCODE_01_H_
...
namespace aoc2019_01 {
  using namespace std;

  inline int calculateCounterUpper(int moduleMass) {
  ...
  unsigned long long solve1() {    
    ...
    while (!cin.eof()) {
      cin >> input;
      result += calculateCounterUpper(input);
    }
    return result;
  }
  ...
  unsigned long long solve2() {    
    ...
    while (!cin.eof()) {
      cin >> input;
      long long acum = input;
      while ((acum = calculateCounterUpper(acum)) > 0) {\
        // cout << input << " - " << acum << endl;
        result += acum;
      }
    }
    return result;
...
#endif /* _2019_ADVENTOFCODE_01_H_ */
