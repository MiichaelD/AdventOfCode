/*
  Link:         http://adventofcode.com/2017/day/15

  Description:  --- Day 15: Dueling Generators ---
      ...
      lowest 16 bits of both values, and keeps track of the number of times those parts of the
      values match.

      The generators both work on the same principle. To create its next value, a generator will
      take the previous value it produced, multiply it by a factor (generator A uses 16807;
      generator B uses 48271), and then keep the remainder of dividing that resulting product by
      ...
      Unfortunately, even though this change makes more bits similar on average, none of these
      values' lowest 16 bits match. Now, it's not until the 1056th pair that the judge finds the
      first match:
      ...
      This change makes the generators much slower, and the judge is getting impatient; it is now
      only willing to consider 5 million pairs. (Using the values from the example above, after five
...
#define _2017_ADVENTOFCODE_15_DUELING_GENERATORS_H_
...
namespace aoc2017_15{
  ...
  // test values:
  // static const uint64_t GEN_A_SEED = 65, GEN_B_SEED = 8921;

  // my values:
  static const uint32_t GEN_A_SEED = 883, GEN_B_SEED = 879;
  static const uint32_t GEN_A_FACTOR = 16807, GEN_B_FACTOR = 48271;
  static const uint64_t DIVIDER = 2147483647; //(2 ^31 -1)

  uint64_t process(uint64_t seed, uint32_t factor) {
    uint64_t result = (seed * factor) % DIVIDER;
    return result;
  }

  bool match(uint64_t valA, uint64_t valB) {
    return (valA & 0xFFFF) == (valB & 0xFFFF);
  }

  uint32_t process(uint32_t times) {
    using namespace std;
    uint64_t seedA = GEN_A_SEED;
    uint64_t seedB = GEN_B_SEED;
    uint32_t total = 0;
    for (int i = 0; i < times; ++i) {
      // cout << "GEN A:\t" << seedA << " -> ";
      seedA = process(seedA, GEN_A_FACTOR);
      // cout << seedA << endl << "GEN B:\t" << seedB << " -> ";
      seedB = process(seedB, GEN_B_FACTOR);
      // cout << seedB << endl;
  ...
  uint32_t processP2(uint32_t times) {
    using namespace std;
    uint64_t seedA = GEN_A_SEED;
    uint64_t seedB = GEN_B_SEED;

    // This can be change to an array of <times> uint64_t
    queue<uint64_t> valuesA;
    queue<uint64_t> valuesB;
    uint32_t total = 0;
    for (int i = 0; valuesA.size() < times || valuesB.size() < times; ++i) {
      if (valuesA.size() < times) {
        seedA = process(seedA, GEN_A_FACTOR);
        // check if number is divisible by 4 by checking last 2 bits = 0;
        if ((seedA & 3) == 0) {
          valuesA.push(seedA);
        }
      }

      if (valuesB.size() < times) {
        seedB = process(seedB, GEN_B_FACTOR);
        // check if number is divisible by 8 by checking last 3 bits = 0;
        if ((seedB & 7) == 0) {
          valuesB.push(seedB);
        }
      }
    }
    cout << "Checking matches of: valuesA = " << valuesA.size();
    cout << ", valuesB = " << valuesB.size() << endl;
    // while (valuesA.size() && valuesB.size()) {
    for (int i = 0; i < times; ++i) {
      seedA = valuesA.front();
      seedB = valuesB.front();
      valuesA.pop();
  ...
  void solve(int part = 1) {
    using namespace std;
    const uint32_t times = part == 1 ? 40000000 : 5000000;
    cout << "Input:\tseedA: " << GEN_A_SEED  << "\tseedB: " << GEN_B_SEED;
    cout << "\tValues to process: " << times << endl;
    uint32_t output = part == 1 ? process(times) : processP2(times);
    cout << "Output:\t" << output << endl;
  }
};
#endif /* _2017_ADVENTOFCODE_15_DUELING_GENERATORS_H_ */
